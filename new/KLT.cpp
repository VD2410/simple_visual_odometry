...
void newpoints(const GpuMat& d_mat, vector<Point2f>& vec)
{
...
void newmasks(const GpuMat& d_mat, vector<uchar>& vec)
{
    vec.resize(d_mat.cols);
...
int main( int argc, char** argv )
{
	...
	if (!(is_gpu)
            ...
            else
            {
                ...
                newpoints(d_vector_key_points, keypoints);
    ...
    cout<<"Total time for both the run："<<time_used.count()<<" seconds."<<endl;
    ...
    t2 = chrono::steady_clock::now();
    time_used = chrono::duration_cast<chrono::duration<double>>( t2-t1 );
    cout<<"Time used for GPU Tracking："<<time_used.count()<<" seconds."<<endl;
    ...
    newpoints(gpu_prev_keypoints,cpu_prev_keypoints);
    newpoints(gpu_next_keypoints,cpu_next_keypoints);
    newpoints(gpu_points_back,cpu_points_back);

    newmasks(d_status,status_fwd);
    newmasks(d_status_back,status_bwd);
